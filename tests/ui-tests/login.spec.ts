    ...
    test(`${key} login is ${userRecord.expect}`, async ({ pageManager }) => {
      ...
      await test.step('Verify result', async () => {
        const assertions = {
          ...
          unsuccessful: () => loginPage.expectError(userRecord.errorText!), // Non-null assertion as errorText is required for unsuccessful logins, it is going to be generated at runtime
        };
        await argosScreenshot(pageManager.getPage(), "Login Page - " + key);
