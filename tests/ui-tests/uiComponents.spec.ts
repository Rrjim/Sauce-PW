...
// run both @block and smoke test with:
...
test.describe.only('Form Layouts page @block', () => {
    ...
    test.only('radio button', async({page}) => {
        ...
        // if the input of checkbox is visually-hidden then the check() won't work
    ...
    test('checkboxes', async({page}) => {
        ...
        // click can flag and unflag a checkbox, check can only flag it, uncheck can unflag it
    ...
    test('tooltips', async({page}) => {
        ...
        page.getByRole('tooltip') // it works only if we have a role tooltip created
    ...
    test('sliders', async({page}) => {
        ...
        // convert gauge div into math coordinate system
        const box = await tempBox.boundingBox()
        // set the (0,0) to the center of the gauge
        const x = box.x + box.width / 2
        const y = box.y + box.height / 2
        // move the mouse into the center
        await page.mouse.move(x, y)
        // click the mouse and hold it
